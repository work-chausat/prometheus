...
var DefaultWaterMark = WaterMark{Low: 4 << 30 /*4GB*/, High: math.MaxInt64}
...
func (s *stripeSeries) gc(mint int64) (map[uint64]struct{}, int, int64) {
	...
	for i := 0; i < s.size; i++ {
		...
		for hash, all := range s.hashes[i] {
			for _, series := range all {
				series.Lock()
				chunksRemoved, baseTime := series.gc(mint)
	...
	return deleted, rmChunks, minTime
}
...
func (s *memSeries) gc(mint int64) (chunksRemoved int, baseTime int64) {
	...
	var (
		j = int((deadline - s.baseTime) / chunkenc.TimeWindowMs)
		k int
		c *memChunk
	)
	...
	return chunksRemoved, s.baseTime
}
